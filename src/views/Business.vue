<template>
  <div class="container">
      ...
      <div>
        <!-- conditional rendering in mobile view -->
          <h1 class="heading-holder">Business Section <i class="fa fa-sack-dollar "></i></h1>
          <div class="main-area">
            <!-- Trending Sidebar -->
            <div id = "trending">
              <div> 
            ...
            <div class="trending-information-section">
              <div class="trending-title-container">
                <a  v-on:click ="pushToArticle(articles.categorySpecified, articles.title, articles.articleId)">
                  <h4 class = "trending-title" v-on:click="postClicked=articles.articleId; logArticleClick()">{{articles.title}}</h4>
                </a>
          </div>
            </div>
            ...
            <div class="article-ticker-div" v-show="!toggleContentBasedCollaborativeBased || isDesktop">
              <!-- Tickers -->
              <tickers v-show="!toggleContentBasedCollaborativeBased || isDesktop"></tickers>
              <!-- OIL GAS CHART -->
              <OilGasChart v-show="!toggleContentBasedCollaborativeBased || isDesktop"></OilGasChart>
          <!-- COMMODITIES CHART -->
          <CommoditiesChart v-show="!toggleContentBasedCollaborativeBased || isDesktop"></CommoditiesChart>
            ...
            <div class = "news-grid" v-show="!toggleContentBasedCollaborativeBased || isDesktop">
              ...
              <div class = "article" v-for="article in displayArticles" v-bind:key="article.id">
                ...
                <a  v-on:click ="pushToArticle(article.category, article.title, article.id)">
                ...
                <div class = "buttons">
                  <button v-on:click="categoryLiked = article.category; postLiked = article.id; handleLikeClick(article.id)"><i class="fa fa-thumbs-up fa-regular" v-bind:class="{'i-changed': articleIsLiked(article.id)}"></i></button>
                  <button v-on:click="categoryLiked = article.category; postDisliked = article.id; handleDislikeClick(article.id)"><i class="fa-regular fa-thumbs-down" v-bind:class="{'i-changed': articleIsDisliked(article.id)}"></i></button>
                  <button v-on:click="categoryLiked = article.category; saved = article.id; handleSaveForLaterClick(article.id)"><i class="fa fa-regular fa-bookmark" v-bind:class="{'i-changed': articleIsSavedForLater(article.id)}"></i></button>
                </div>
              </div>
            </div>
          ...
          <i class="fa-solid fa-arrow-right fa-lg toggle-main-content-right" v-if="!toggleContentBasedCollaborativeBased" v-on:click="toggleContentBasedCollaborativeBased = true"></i>
            ...
            <fwlsRecommendationsSideBarVue v-show="toggleContentBasedCollaborativeBased && !isDesktop"></fwlsRecommendationsSideBarVue>
            <fwlsRecommendationsSideBarVue v-if="isDesktop"></fwlsRecommendationsSideBarVue>
      </div>
      </div>
  ...
  import OilGasChart from '@/components/OilGaschart.vue';
  import Tickers from '@/components/Tickers.vue';
import fwlsRecommendationsSideBarVue from '@/components/FwlsRecommendationsSideBar.vue';
import BottomNavigation from '@/components/BottomNavigation.vue'
  ...
  export default {
    ...
    beforeMount() {
      ...
      ArticleService.listBusinessArticles().then(response => {
  ...
  displayArticles(){
    ...
    handleDislikeClick() {
if (this.userDislikes.some(dislike => dislike.id === this.postDisliked)) {
  ArticleDataService.undislikeAPost(this.postDisliked).then(response => {
    if (response.status === 200) {
      this.userDislikes = this.userDislikes.filter(dislike => dislike.id !== this.postDisliked);
  ...
  if (this.userLikes.some(like => like.id === this.postDisliked)) {
    ArticleDataService.unlikeAPost(this.postDisliked).then(response => {
      if (response.status === 200) {
        this.userLikes = this.userLikes.filter(like => like.id !== this.postDisliked);
        
        this.isLiked = false;
      }
    });
  }
  if (this.usersSavedForLater.some(saved => saved.id === this.postDisliked)) {
    ArticleDataService.removeSaveForLater(this.postDisliked).then(response => {
      if (response.status === 200) {
        this.usersSavedForLater = this.usersSavedForLater.filter(saved => saved.id !== this.postDisliked);
...
//saves a post, deletes in sql or posts to sql
      handleSaveForLaterClick(){
        if(this.usersSavedForLater.some(saved => saved.id === this.saved)){
          ArticleDataService.removeSaveForLater(this.saved).then(response=> {
            if(response.status === 200 ){
              this.usersSavedForLater = this.usersSavedForLater.filter(saved => saved.id !== this.saved);
          ...
          if(this.userLikes.some(like => like.id === this.saved)){
          ArticleDataService.unlikeAPost(this.saved).then(response=> {
            if(response.status === 200 ){
              this.userLikes = this.userLikes.filter(like => like.id !== this.saved);
        ...
        if(this.userDislikes.some(like => like.id === this.saved)){
          ArticleDataService.undislikeAPost(this.saved).then(response=> {
            if(response.status === 200 ){
              this.userDislikes = this.userDislikes.filter(like => like.id !== this.saved);
