<template>
  <div class="container">
      ...
      <div>
        <!-- conditional rendering in mobile view -->
          <h1 class="heading-holder" v-if="newsOrTrendingClicked">Your feed <i class="fa-solid fa-house"></i></h1>
          <h1 class="heading-holder" v-if="!newsOrTrendingClicked">Trending <i class="fa fa-fire"></i> </h1>
          <div class="main-area">
            <!-- Trending Sidebar -->
            <div id = "trending" v-bind:class="{hidden: !isDesktop}">
              <div> 
            ...
            <div class = "news-grid" v-show="!toggleContentBasedCollaborativeBased || isDesktop">
              ...
              <div class = "article" v-for="article in displayArticles" v-bind:key="article.id">
                <div>
                  <img class="article-image" :src=article.urlToImage alt="article-image" preload="auto">
            ...
            <ContentBasedSuggestionsSideBar v-show="toggleContentBasedCollaborativeBased && !isDesktop"></ContentBasedSuggestionsSideBar>
            <ContentBasedSuggestionsSideBar v-if="isDesktop"></ContentBasedSuggestionsSideBar>
          </div>
      </div>
  ...
  import ContentBasedSuggestionsSideBar from '@/components/ContentBasedSuggestionsSideBar.vue';
  ...
  export default {
    ...
    components: {
      navBar,
      MobileNavbar,
      ContentBasedSuggestionsSideBar,
      BottomNavigationVue
    },
    data() {
      return {
    ...
    created() {
      ...
      const token = this.getAccessTokenFromURL().accessToken;
      const idToken = this.getAccessTokenFromURL().idToken;
        if (token && idToken) {
          console.log(token)
          console.log(idToken)
          axios.post('http://localhost:9000/validateGoogleTokenSignUp', null, {
            headers: {
        ...
        .catch(error => {
            ...
            this.$store.commit("SET_USER", this.currentUserName);
      }
      ArticleDataService.getUsername().then(response => {
            this.currentUsername = response.data
      }),
      //calls current pages articles
      ArticleService.listCollaborativeArticles(this.currentUser).then(response => {
      this.curatedContentBasedArticles = response.data;
    }),
    ...
    ArticleService.listTrendingArticles().then(response => {
      ...
      console.log(JSON.stringify(this.trendingArticles))
    });
  },  
    mounted() {
  ...
  displayArticles(){
  return this.curatedContentBasedArticles.slice(this.start, this.end);
  },  
