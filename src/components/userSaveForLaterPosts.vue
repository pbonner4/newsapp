<template>
    <div class="save-for-later-posts-container">
                <h1 v-on:click="showSaveForLaters = true" v-show="showSaveForLaters == false">Show Posts Saved For Later <i class="fa-solid fa-caret-right"></i></h1>
                <h1 v-on:click="showSaveForLaters = false" v-show="showSaveForLaters == true">Hide Posts Saved For Later <i class="fa-solid fa-caret-down"></i></h1>
        <div class = "save-for-later-article-container" v-show="showSaveForLaters == true">
                ...
                <div class = "article" v-for="article in displayArticles" v-bind:key="article.id">
                  ...
                  <div class = "buttons">
                    <button v-on:click="postLiked = article.id; handleLikeClick(article.id)"><i class="fa fa-regular fa-bookmark" v-bind:class="{'i-changed': articleIsSavedForLater(article.id)}"></i></button>
                  </div>
                </div>
            </div>
            <div class="show-more-button" v-on:click="end = usersSavedForLaterLength" v-if="end == 6 && showSaveForLaters == true"> 
    ...
    export default {
      ...
      ArticleDataService.listSaveForLaters().then(response => {
        this.userSaveforLaters = response.data
    ...
    return articleId => this.userSaveforLaters.some(saved => saved.id === articleId);
  },
    displayArticles() {
      return this.userSaveforLaters.slice(this.start, this.end)
    },
    usersSavedForLaterLength(){
      return this.userSaveforLaters.length()
    }
  },
      methods: {
  //saves a post, deletes in sql or posts to sql
     ...
     const event = {
            ...
            LoggerService.logger(event).then(response => {
              if(response.status == 200){
                ...
                console.logle.log('save logged')
              } else {
                console.logle.logle.logle.log('not working')
              }
            })
                }
      },
        logArticleClick(){
            ...
            LoggerService.logger(event).then(response => {
              if(response.status == 200){
                ...
                console.logle.log('title logged')
              } else {
    ...
    .save-for-later-posts-container{
        width: 100%;
        background-color: #eeeff1;
    }
    .save-for-later-article-container{
        margin: 0px;
        width: 90%;
      height: fit-content;
      padding: 3% 3% 3% 3%;
