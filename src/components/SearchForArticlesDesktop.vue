<template>
    <div> 
        <div>
            ...
            <div v-if="!searchedArticles.length == 0"> 
                <h3>Searched: "{{ this.$route.params.term }}"</h3>
    ...
    export default {
        ...
        ArticleService.listSearchedArticles(this.$route.params.term).then(response => {
  ...
  ArticleService.listSearchedArticles(to.params.term).then(response => {
    this.searchedArticles = response.data;
    window.scrollTo(0, 0);
    next();
  }).catch(error => {
